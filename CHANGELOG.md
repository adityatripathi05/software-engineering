...
**Batch 2: scale and trust — 55 cells across 2 notebooks.** Folder total so far:
**98 cells, 4 notebooks**.

### `18.3 Pagination, Rate Limits and Retries.ipynb` — **NEW**, 27 cells
- **Three pagination styles** against a server that implements all of them — page number with
  an RFC 8288 `Link` header, cursor, and a deliberately broken paginator. `response.links` is
  highlighted as the feature nobody knows about, with 🔴 `urljoin` because a `Link` URL may be
  **relative**.
- 🔴 **Pagination belongs in a generator** (**4.3**), measured: taking three items costs
  **one request, not five**, because the generator stops when the caller does. Then the same
  generator consumed fully (5 requests, 47 items) and filtered.
- 🔴 **The infinite-pagination bug, run to the edge.** `/jobs/broken` always reports a next
  page; the guarded version stops cleanly at 47 items while the naive one is still asking for
  page 9 of an empty collection when the cell cuts it off. Three guards given: empty page, no
  new IDs, and a hard page cap.
- **Rate limits** honoured from the response: `Retry-After` obeyed across two `429`s, with
  `X-RateLimit-Remaining` shown, and the point that `Retry-After` is an instruction rather than
  advice.
- 🔴 **Backoff with jitter, and the thundering herd simulated.** 200 clients retrying at
  attempt 3: pure exponential gives **1 distinct wait time — all 200 in the same second**;
  full jitter gives **9 distinct times with at most 34 in any one**. That contrast is the whole
  argument for jitter, and it builds directly on **15.1**'s `retry_delay`.
- **A retry-policy table generated by a function**, covering ten method/status/error
  combinations. The two `POST` + `503` rows differ only by an idempotency key and the decision
  flips — which motivates the next section.
- **`urllib3.Retry` on an `HTTPAdapter`**: the caller makes one call, urllib3 sends **three**
  HTTP requests to a flaky endpoint, and a `403` is returned immediately because it is not in
  `status_forcelist`. With the note that `allowed_methods` excluding `POST` is the correct
  default.
- 🔴 **Idempotency keys, demonstrated both ways.** With a key, the retry returns the **original**
  record and `X-Replayed: true`; without one, the same two calls create **`build-902` and
  `build-903`** — two records from one intent. Plus the warning that generating a fresh key per
  attempt defeats the mechanism entirely.
- Circuit breakers, with the three states and the argument that the value is **failing fast**.

> 🔴 **Three defects caught by reading the output.** The `Link`-header paginator raised
> `MissingSchema`, because the header is relative and `requests` needs an absolute URL — fixed
> with `urljoin`, which is what real clients need anyway. A generator demo wrapped a non-generator
> in `list()` and raised `TypeError`. And worst: the idempotency cell **claimed** two records
> were created without a key while printing the same id twice — my fake server only incremented
> its counter when a key was present, so the demonstration contradicted its own point.

### `18.4 Validating What You Receive - pydantic.ipynb` — **NEW**, 28 cells
Closes the gap **16.1** opened ("annotations are not enforcement") and pays off promises from
**8.3**, **16.1** and **16.2**.

- 🔴 **The trust boundary**, then what happens without one: the same three payloads through
  naive code produce a `TypeError` about strings and a `KeyError` — **neither of which mentions
  the API**. And the note that the non-crashing case is worse: `attempts` was the string
  `"three"`, which would have been stored.
- **`BaseModel` reporting every problem at once** — four errors from the drifted payload,
  including `owner.email` nested one level down — against an `if` ladder that stops at the first.
- 🔴 **Coercion vs strict**: lax mode turns `"3"` into `3` and `"0.5"` into `0.5`, **refuses
  `"3.7"` as an int** rather than truncating, and **accepts `True` as `1`** because `bool` is an
  `int`. Strict mode refuses all of it.
- **Constraints via `Annotated[..., Field(...)]`**, which is exactly the shape **16.2** left as
  an exercise with the remark *"you have just written a tiny pydantic"* — here with four
  constraints violated at once and each reported by name.
- **Nested error paths** — `items.1.owner.email` — and then 🔴 the deliberate decision between
  all-or-nothing and best-effort parsing, with the warning that silently dropping records is the
  worst of the three options.
- **Extra fields** in all three modes, with the rule: `ignore` for **someone else's** API so
  their new field cannot break you, `forbid` for **your own** config so a typo fails loudly.
- **Aliases**, and 🔴 `model_dump(by_alias=True)` as a common source of `422`s on write endpoints.
- 🔴 **The revised rule**: **16.2** said *TypedDict at the boundary, dataclass inside*; with
  validation available it becomes **`BaseModel` at the boundary, `dataclass` inside**. Shown
  end to end — a `frozen=True` domain object that raises `FrozenInstanceError` on mutation, and
  bad data failing at the boundary with 4 errors *before any business logic runs*.
