# 03 FastAPI

Prerequisites: modern-python 5.4, 12.5, 18.4 · Depends on backend modules: 01, 02

Read top to bottom. Every notebook follows [AUTHORING-GUIDE.md](../AUTHORING-GUIDE.md).

| # | Notebook | Level | Status |
|---|---|---|---|
| 03.1 | [Application structure and ASGI - what uvicorn actually does](03.1-application-structure-and-asgi.md) | Beginner | draft |
| 03.2 | [Routing and parameters](03.2-routing-and-parameters.md) | Beginner | draft |
| 03.3 | [Request/response models](03.3-request-response-models.md) | Beginner | draft |
| 03.4 | [Pydantic v2 in depth - validation modes, serialisation, settings](03.4-pydantic-v2-in-depth.md) | Intermediate | draft |
| 03.5 | [Dependency injection](03.5-dependency-injection.md) | Intermediate | draft |
| 03.6 | [Sync vs async endpoints - the threadpool](03.6-sync-vs-async-endpoints.md) | Intermediate | draft |
| 03.7 | [Middleware](03.7-middleware.md) | Intermediate | todo |
| 03.8 | [Exception handling and error mapping](03.8-exception-handling.md) | Intermediate | todo |
| 03.9 | [Lifespan and application lifecycle](03.9-lifespan.md) | Intermediate | todo |
| 03.10 | [BackgroundTasks vs real workers](03.10-backgroundtasks-vs-workers.md) | Intermediate | todo |
| 03.11 | [File uploads and streaming responses](03.11-file-uploads-and-streaming.md) | Intermediate | todo |
| 03.12 | [WebSockets basics](03.12-websockets-basics.md) | Intermediate | todo |
| 03.13 | [OpenAPI customisation](03.13-openapi-customisation.md) | Beginner | todo |
| 03.14 | [Production deployment - uvicorn/gunicorn workers, signals, behind a proxy](03.14-production-deployment.md) | Advanced | todo |
